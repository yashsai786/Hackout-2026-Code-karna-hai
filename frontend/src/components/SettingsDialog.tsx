...
export const SettingsDialog = () => {
  const { connected, keyInfo, models, modelsLoading, modelsError, defaultModel, setDefaultModel, connect, disconnect, loadModels, toolModelCount } = useSettings();
  ...
  const filtered = query ? pool.filter(m => m.id.toLowerCase().includes(query) || m.name.toLowerCase().includes(query)) : pool;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="app-dialog settings-dialog" data-testid="settings-dialog">
        ...
        <DialogDescription data-testid="settings-description">Session-only preferences. Your key is never sent to a Leakpoint server and is never stored; it clears on refresh.</DialogDescription>
        ...
        <div className="settings-body">
          ...
          <section className="settings-block settings-block-grow" data-testid="settings-model">
            <header>
              ...
              <div><h3>Default model</h3><p>{modelsLoading ? 'Loading the live catalogue…' : modelsError ? 'Could not load models.' : `${toolModelCount} of ${models.length} models can call tools.`}</p></div>
            </header>
            <label className="model-filter"><input type="checkbox" data-testid="models-tools-only" checked={toolsOnly} onChange={e => setToolsOnly(e.target.checked)} /><span>Only models that can call tools</span></label>
              ...
              {modelsError ? <p className="model-empty">{modelsError} <button className="text-link" data-testid="retry-models" onClick={loadModels}>Retry</button></p>
                ...
                : shown.map(m => (
                  <button key={m.id} className={`model-row ${defaultModel === m.id ? 'selected' : ''}`} data-testid={tid(m.id)} onClick={() => setDefaultModel(m.id)}>
                    ...
                    <span className="model-row-end">{!supportsTools(m) && <Tag id={`model-notools-${tid(m.id)}`} tone="warning">No tools</Tag>}{defaultModel === m.id && <Check size={16} />}</span>
                  </button>
                ))}
            </div>
          ...
          <section className="settings-block" data-testid="settings-data">
            <header>
              <span className="settings-block-ic"><RotateCcw size={18} /></span>
              <div><h3>Session data</h3><p>Your factories, baselines, records and ledger are kept in this browser so a reload does not lose them.</p></div>
            </header>
            <div className="settings-actions">
              <Btn data-testid="reset-session" onClick={() => { if (confirm('Reset every factory, baseline, record and ledger entry back to the demonstration dataset?')) resetSessionData(); }}>Reset demonstration data</Btn>
            </div>
          </section>
        </div>
      </DialogContent>
