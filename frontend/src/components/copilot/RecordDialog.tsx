    ...
    >
      <DialogContent className="app-dialog" data-testid="copilot-record-dialog">
        ...
        {preview && (
          <div className="copilot-record-figures">
            <div>
              ...
              <strong>{fmt(Number(preview.reduction_tco2e_yr))} tCO₂e/yr</strong>
            </div>
            <div>
              <span>Net operating savings</span>
              <strong>{money(Number(preview.net_operating_savings_inr_yr))}/yr</strong>
            </div>
            <div>
              <span>Upfront capex</span>
              <strong>{money(Number(preview.capex_inr_one_off))}</strong>
            </div>
            <div>
              <span>Simple payback</span>
              <strong>
                {preview.payback_months === null
                  ? 'No break-even'
                  : preview.payback_months === 0
                    ? 'Immediate'
                    : `${Number(preview.payback_months).toFixed(1)} months`}
              </strong>
            </div>
          </div>
        )}
        ...
        <div className="dialog-actions">
          ...
          <Btn variant="primary" data-testid="copilot-confirm-record" onClick={confirmPending}>
            <BookOpen size={16} />
            Record estimate
          </Btn>
        </div>
