...
export const Transcript = () => {
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="group"
      aria-label={`Copilot conversation, ${turns.length} messages`}
    >
      {cold && (
          ...
          {mode === 'disconnected' ? (
            <div className="copilot-connect" data-testid="copilot-connect">
              ...
              <div>
                ...
                <p>
                  The Copilot uses the OpenRouter key from Settings. Figures always come from this session's
                  calculator; the model only phrases them.
                </p>
              </div>
              <Btn data-testid="copilot-open-settings" onClick={() => setSettingsOpen(true)}>
      ...
      {turns.map(turn => (
        <article
          className={`copilot-msg ${turn.role}`}
          key={turn.id}
          aria-label={turn.role === 'user' ? 'Your question' : 'Copilot answer'}
        >
          <div className="copilot-bubble">
            <RichText text={turn.text} />
            {turn.stopped && (
              <Tag id={`copilot-stopped-${turn.id}`} tone="warning">
                Stopped
              </Tag>
            )}
            {turn.narrated && (
              <Tag id={`copilot-narrated-${turn.id}`} tone="warning">
      ...
      {running &&
        (streaming ? (
          <article className="copilot-msg assistant" aria-label="Copilot answer in progress">
            <div className="copilot-bubble">
          ...
          <div className="copilot-running" data-testid="copilot-running">
            <span className="copilot-spinner" aria-hidden="true" />
            {toolNow ? (
              <span>
                Running <code className="mono">{toolNow}</code>…
              </span>
            ) : (
              <span>Thinking…</span>
            )}
      ...
      {running && streaming && toolNow && (
        <div className="copilot-running" data-testid="copilot-running-tool">
          <span className="copilot-spinner" aria-hidden="true" />
          <span>
            Running <code className="mono">{toolNow}</code>…
          </span>
      ...
      {error && (
        <div className="copilot-error" role="alert" data-testid="copilot-error">
          ...
          <div>
            <strong>The Copilot could not answer</strong>
            <p>{error} Your session data is unchanged.</p>
          </div>
        </div>
      )}
    </div>
