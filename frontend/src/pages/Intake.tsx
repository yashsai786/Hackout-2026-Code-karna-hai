...
export default function Intake() {
  const { factories, addIntake } = useSession(),
  ...
  const [quantity, setQuantity] = useState(''),
    ...
    [period, setPeriod] = useState('2025-12');
  const [cancelled, setCancelled] = useState(false),
    [dragOver, setDragOver] = useState(false);
  const timer = useRef<number | null>(null),
    saved = useRef(false),
    fileInput = useRef<HTMLInputElement>(null);
  const sample = extractionSamples.find(s => s.id === sampleId)!,
    factory = factories.find(f => f.id === factoryId);
  const invalid =
    !quantity.trim() ||
    !factor.trim() ||
    !rate.trim() ||
    [Number(quantity), Number(factor), Number(rate)].some(n => !Number.isFinite(n) || n < 0) ||
    !/^\d{4}-\d{2}$/.test(period);
  let estimate = { emissions: 0, cost: 0 };
  let calculationError = invalid;
  if (!invalid) {
    try {
      estimate = extractEstimate(Number(quantity), Number(factor), Number(rate));
    } catch {
  ...
  const confirm = () => {
    if (calculationError || !factory || saved.current) return;
    saved.current = true;
    addIntake({
      factoryId,
      sample: sample.name,
    ...
    <>
      ...
      <div className="stepper" aria-label="Intake progress" data-testid="intake-progress">
        {['Choose a source', 'Review extraction', 'Confirm estimate'].map((s, i) => (
          <div key={s} className={(stage === 'saved' ? 2 : stage === 'review' ? 1 : 0) >= i ? 'current' : ''}>
            <span>
              {stage === 'saved' || (stage === 'review' && i === 0) ? <Check size={15} /> : `0${i + 1}`}
            </span>
            <strong>{s}</strong>
      ...
      {!factory && (
        <div className="notice warning" role="alert" data-testid="intake-invalid-factory">
          The requested factory is unavailable. Select a factory below to continue.
        </div>
      )}
      {stage === 'saved' ? (
        <div className="intake-success" data-testid="intake-success">
          ...
          <div className="eyebrow">SOURCE RECORD CONFIRMED</div>
          <h2 data-testid="intake-success-heading">One source. A more informed estimate.</h2>
          <p>
            <strong>{fmt(estimate.emissions, 2)} tCO₂e</strong> recorded for {factory?.name} · {period}.
          </p>
          <Notice id="intake-baseline-unchanged">
            This record covers <strong>one source for {period}</strong>. It does not change the annual
            baseline on its own
            {factory?.baseline === null
              ? ', which is still Awaiting baseline'
              : `, which stays at ${fmt(factory?.baseline || 0)} tCO₂e`}
            . Apply it on the process page to set or revise the baseline.
          </Notice>
          <div className="button-row">
            ...
            <Link
              ...
              to={`/factories/${factoryId}/profile`}
              data-testid="intake-apply-baseline"
            >
              {factory?.baseline === null ? 'Set the baseline' : 'Revise the baseline'}
              <ArrowRight size={16} />
            </Link>
            <Link
              className="app-btn outline"
              to={`/factories/${factoryId}`}
              data-testid="intake-view-factory"
            >
              View factory
              <ArrowRight size={16} />
            </Link>
        ...
        <div className="intake-layout">
          <section>
            <SectionHeading
              ...
              note={
                stage === 'review'
                  ...
                  : 'A record belongs to one factory and one reporting period'
              }
            />
            <label className="field-label">
              Factory
              <select
                data-testid="intake-factory"
                value={factoryId}
                disabled={stage !== 'select'}
                onChange={e => setFactoryId(e.target.value)}
              >
                {!factory && <option value={factoryId}>Select a valid factory</option>}
                {factories.map(f => (
              ...
              <>
                <div
                  ...
                  onDrop={e => {
                    e.preventDefault();
                    setDragOver(false);
                    capture(e.dataTransfer.files[0]);
                  }}
                  data-testid="file-drop-zone"
                >
                  <UploadCloud size={33} strokeWidth={1.4} />
                  ...
                  />
                  <div className="button-row">
                    <Btn data-testid="browse-file" onClick={() => fileInput.current?.click()}>
                ...
                <div className="sample-grid">
                  {extractionSamples.map((s, i) => {
                    const Icon = sampleIcons[i];
                    return (
                      <button
                        className={`sample-card ${sampleId === s.id ? 'selected' : ''}`}
                        key={s.id}
                      ...
                      >
                        ...
                        <span>{s.kind}</span>
                        {sampleId === s.id && <Check size={15} className="sample-check" />}
                      </button>
                    );
                  })}
                </div>
                {cancelled && (
                  <p role="status" className="cancelled-message" data-testid="extraction-cancelled">
                    Extraction cancelled. No source record was saved.
                  </p>
                )}
            ...
            {stage === 'review' && (
              <>
                <div className="extraction-source" data-testid="extraction-source">
                  <FileText size={22} />
                  <div>
                ...
                <div className="form-grid extraction-fields">
                  <label>
                    ...
                    <input
                      ...
                      min="0"
                      step="any"
                      data-testid="extraction-quantity"
                      value={quantity}
                  ...
                  <label>
                    ...
                    <input
                      type="month"
                      data-testid="extraction-period"
                      value={period}
                      onChange={e => setPeriod(e.target.value)}
                    />
                    <small>Single-period record</small>
                  </label>
                </div>
                {calculationError && (
                  <p role="alert" className="field-error" data-testid="extraction-validation-error">
                    Enter non-negative finite numbers and a reporting month.
                  </p>
                )}
                ...
                <div className="form-bottom">
                  ...
                  <Btn
                    ...
                    disabled={calculationError || !factory}
                    onClick={confirm}
                  >
                    Confirm source estimate
                    <Check size={16} />
                  </Btn>
                </div>
              </>
          ...
          <aside className="intake-summary">
            ...
            <h2 data-testid="intake-summary-title">{sample.name}</h2>
            <Tag id="intake-estimate-status" tone={stage === 'review' ? 'blue' : 'neutral'}>
              {stage === 'review' ? 'Ready for review' : 'Awaiting extraction'}
            </Tag>
            <div className="estimate-metric">
              <span>Source emissions</span>
              <strong data-testid="intake-estimate-emissions">
                {stage === 'review' && !calculationError ? fmt(estimate.emissions, 2) : '—'}
                <small>tCO₂e</small>
              </strong>
            </div>
            <div className="estimate-metric">
              <span>Source operating cost</span>
              <strong data-testid="intake-estimate-cost">
                {stage === 'review' && !calculationError ? money(estimate.cost) : '—'}
              </strong>
            </div>
            <div className="estimate-formula" data-testid="intake-formula">
              <span>CALCULATION</span>
              <p>Activity quantity × emission factor</p>
              <p>Activity quantity × unit cost</p>
            </div>
          </aside>
        </div>
      )}
