...
import { exposure, intensity, scenario, eligibleFor } from '../domain/calculations';
import {
  PageHeading,
  Stat,
  Tag,
...
export default function FactoryDetail() {
  const { id } = useParams(),
    { factories, intake } = useSession(),
    f = factories.find(f => f.id === id);
  if (!f) return <NotFound kind="Factory" to="/factories" label="Factory index" />;
  const records = intake.filter(i => i.factoryId === id),
    annualExposure = exposure(f);
  return (
    <>
      <Link to="/factories" className="back-link" data-testid="factory-back">
        <ArrowLeft size={15} />
        Factory index
      </Link>
      <PageHeading
      ...
      {f.baseline === null ? (
        <>
          <Notice id="awaiting-baseline-notice">
            This factory is <strong>Awaiting baseline</strong>. Intake records remain separate source
            estimates. No coordinates, emissions total, ranking, or recommendation is assumed.
          </Notice>
          <div className="awaiting-baseline">
            <span className="factory-sector-icon">
              <Flame size={30} />
            </span>
            <h2 data-testid="awaiting-heading">A reliable baseline comes first.</h2>
            <p>
              Annual production, complete energy and process records, and an agreed reporting boundary are
              still required.
            </p>
            <Link
              to={`/factories/${f.id}/profile`}
              className="app-btn primary"
              data-testid="awaiting-add-data"
            >
              Add source data
              <ArrowUpRight size={16} />
            </Link>
        ...
        <>
          <div className="stats-grid">
            <Stat
              ...
              value={compact(f.baseline)}
              unit="tCO₂e"
              note="Scope 1 + 2 · FY 2025"
            />
            <Stat
              id="detail-intensity"
              label="Emissions intensity"
              value={fmt(intensity(f)!, 2)}
              unit="tCO₂e/t"
              note={`${fmt(f.production!)} tonnes annual output`}
            />
            <Stat
              id="detail-exposure"
          ...
          <div className="two-columns">
            <section className="border-section">
              <SectionHeading title="Emissions hotspots" note="Annual contribution by source" />
              {[...sources]
                .sort((a, b) => f.hotspots[b] - f.hotspots[a])
                .map(s => {
                  const Icon = icons[s];
                  return (
                    <div className="hotspot" key={s} data-testid={`hotspot-${s}`}>
                      <div className="hotspot-header">
                        ...
                        <strong>
                          {fmt(f.hotspots[s])} <small>tCO₂e</small>
                        </strong>
                        <span>{((f.hotspots[s] / f.baseline!) * 100).toFixed(0)}%</span>
                      </div>
                      <div className="hotspot-track">
                        <div
                          className={`hotspot-fill source-${s}`}
                          style={{ width: `${(f.hotspots[s] / f.baseline!) * 100}%` }}
                        />
                      </div>
                    </div>
                  );
          ...
          <section className="full-section">
            <SectionHeading
              ...
              note={`${f.sector} production flow · schematic, not an engineering diagram`}
            />
            <div
              className="process-flow"
              tabIndex={0}
              role="group"
              aria-label={`${f.sector} production flow, ${processFlows[f.sector].length} stages`}
            >
              {processFlows[f.sector].map((s, i) => (
                <div className="process-step" key={s} data-testid={`process-step-${i}`}>
                  <span className="process-index">0{i + 1}</span>
                  <strong>{s}</strong>
                  {i < processFlows[f.sector].length - 1 && <ArrowRight size={18} />}
                </div>
              ))}
            </div>
          </section>
          <div className="two-columns">
            <section className="border-section">
              ...
              <dl className="assumptions-list">
                {sources.map(s => (
                  <div key={s} data-testid={`cost-input-${s}`}>
                    <dt>{sourceLabels[s]} cost / year</dt>
                    <dd>{money(f.costs[s])}</dd>
                  </div>
                ))}
                <div data-testid="waste-input">
                  <dt>Waste generated / year</dt>
                  <dd>{fmt(f.wasteTonnes)} tonnes</dd>
                </div>
                <div data-testid="production-input">
                  <dt>Annual production</dt>
                  <dd>{fmt(f.production!)} tonnes</dd>
                </div>
              </dl>
            </section>
            <section className="border-section">
          ...
          <section className="full-section">
            ...
            <div className="recommendation-grid">
              {interventions
                ...
                .sort((a, b) => scenario(f, [b], 100).reduction - scenario(f, [a], 100).reduction)
                .slice(0, 3)
                .map(i => (
                  <Link
                    ...
                    data-testid={`factory-recommendation-${i.id}`}
                  >
      ...
      <section className="full-section">
        ...
        {records.length ? (
          <div className="intake-record-list">
            {records.map(r => (
              <div key={r.id} className="intake-record" data-testid={`source-record-${r.id}`}>
                <span>
                  {r.sample}
                  <small>
                ...
                <strong>
                  {fmt(r.emissions, 2)} tCO₂e <small>source estimate only</small>
                </strong>
              </div>
            ))}
          </div>
        ) : (
          <p className="muted body-small" data-testid="no-intake-records">
            No source records added in this session.
          </p>
        )}
      </section>
    </>
