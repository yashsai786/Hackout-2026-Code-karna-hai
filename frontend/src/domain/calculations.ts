...
export const intensity = (f: Factory) =>
  f.baseline !== null && f.production && f.production > 0 ? f.baseline / f.production : null;
  ...
  return items.every((a, i) =>
    items.every(
      (b, j) =>
        ...
        (a.id !== b.id &&
          a.source !== b.source &&
          a.compatibleWith.includes(b.id) &&
          b.compatibleWith.includes(a.id)),
    ),
  );
}
...
export const CAPEX_SCALE_EXPONENT = 0.7;
/**
...
/**
 * Annual return on the upfront capital, as a percentage. Defined here rather than at each call site
 * so the Interventions cards, the scenario screen and the Copilot can never quote different figures
 * for the same measure.
 *
 * Null has a meaning: either the measure does not pay for itself (no return to express), or it needs
 * no capital at all (a ratio over zero is not a number a plant manager can act on). Callers label
 * those two cases differently — see Interventions.tsx.
 */
export const roiPercent = (operatingSavings: number, capex: number) =>
  capex <= 0 || operatingSavings <= 0 ? null : (operatingSavings / capex) * 100;
...
export function scenario(f: Factory, items: Intervention[], adoption: number) {
  ...
  if (!Number.isFinite(adoption) || adoption < 0 || adoption > 100)
    throw new Error('Adoption must be between 0 and 100.');
  if (!items.length || !compatible(items) || items.some(i => !eligibleFor(f, i)))
    throw new Error('This combination is not compatible.');
  const scale = adoption / 100;
  const reduction = Math.min(
    f.baseline,
    sum(items.map(i => Math.min(f.hotspots[i.source], f.hotspots[i.source] * i.reductionRate * scale))),
  );
  ...
  const roi = roiPercent(operatingSavings, capex);
  return {
    reduction,
    grossSavings,
    opex,
    ...
    paybackMonths,
    roiPercent: roi,
    cashflow: Array.from({ length: 37 }, (_, month) => ({
      month,
      value: -capex + (operatingSavings / 12) * month,
    })),
    remaining: Math.max(0, f.baseline - reduction),
  };
}
export const exposure = (f: Factory) =>
  f.baseline === null || !['Steel', 'Cement'].includes(f.sector)
    ? null
    : f.baseline * f.exportShare * reference.carbonEUR * reference.eurINR;
export const readinessCount = (f: Factory) => Object.values(f.readiness).filter(Boolean).length;
export const CREDIT_FACTOR = 0.7;
  ...
  const candidates =
    f.baseline === null
      ...
      : interventions
          ...
          .map(i => ({ item: i, result: scenario(f, [i], 100) }))
          .sort((a, b) => b.result.reduction - a.result.reduction);
  const best = candidates[0] ?? null,
    volume = Math.floor((best?.result.reduction || 0) * CREDIT_FACTOR);
  return {
    best,
    candidates,
    factor: CREDIT_FACTOR,
    volume,
    valueLowINR: volume * reference.creditLowINR,
    valueHighINR: volume * reference.creditHighINR,
  };
}
export function extractEstimate(quantity: number, factor: number, costRate: number) {
  if ([quantity, factor, costRate].some(n => !Number.isFinite(n) || n < 0))
    throw new Error('Enter non-negative, finite values.');
...
export function portfolioTotals(entries: LedgerEntry[], factories: Factory[]) {
  ...
  entries.forEach(e => {
    ...
    e.interventionIds.forEach(id => {
      ...
      if (!item) return;
      const s = scenario(f, [item], e.adoption),
        key = `${f.id}:${item.source}`,
        previous = grouped.get(key);
      if (!previous || s.reduction > previous.reduction)
        grouped.set(key, { reduction: s.reduction, savings: s.operatingSavings });
    });
  });
  return {
    reduction: sum([...grouped.values()].map(g => g.reduction)),
    operatingSavings: sum([...grouped.values()].map(g => g.savings)),
    realisedSavings: sum(entries.map(e => e.realisedSavings)),
  };
}
export function csvExport(entries: LedgerEntry[], factories: Factory[]) {
  ...
  const rows: (string | number)[][] = [
    [
      ...
      'Estimated operating savings INR/year',
      'Upfront capex INR',
      'Realised savings INR',
      'Status',
      'Registry verified',
      'Simulation',
    ],
  ...
  entries.forEach(e =>
    rows.push([
      e.id,
      e.createdAt,
      factories.find(f => f.id === e.factoryId)?.name || 'Unknown',
      e.interventionIds.map(id => interventions.find(i => i.id === id)?.name).join(' + '),
      e.adoption,
      e.reduction.toFixed(2),
      e.operatingSavings.toFixed(2),
      e.capex,
      e.realisedSavings,
      e.status,
      'No',
      'Yes — illustrative estimate only',
    ]),
  );
