...
export function scenario(f: Factory, items: Intervention[], adoption: number) {
  ...
  if (!items.length || !compatible(items) || items.some(i => !i.sectors.includes(f.sector))) throw new Error('This combination is not compatible.');
  ...
  const capex = sum(items.map(i => i.capex));
