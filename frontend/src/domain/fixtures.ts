...
export const reference = { carbonEUR: 75, eurINR: 90, creditLowINR: 600, creditHighINR: 1500, gridFactor: 0.000716, coalFactor: 2.42, wasteFactor: 0.45,
  ...
  gridRateINR: 7.5, coalRateINR: 8200, wasteRateINR: 1400, processRateINR: 600};
...
const materialTemplates: Record<Sector, { name: string; perTonneOfProduct: number; costPerTonne: number; recycledShare: number }[]> = {
  Steel: [
...
export const materialsFor = (sector: Sector, production: number): MaterialStream[] =>
  materialTemplates[sector].map(m => ({ name: m.name, tonnesPerYear: Math.round(production * m.perTonneOfProduct), costPerTonne: m.costPerTonne, recycledShare: m.recycledShare }));
...
export const historyFor = (baseline: number) => historyWeights.map((w, i) => ({ month: MONTHS[i], emissions: Math.round(baseline * w) }));
export const factories: Factory[] = seeds.map(([id, name, city, state, sector, lat, lng, baseline, production, exportShare], index) => {
  ...
  const coalTonnes = hotspots.fuel / reference.coalFactor, kWh = hotspots.electricity / reference.gridFactor;
  ...
  return { id, name, city, state, sector, coordinates: [lat, lng], baseline, production,
    ...
    costs: { fuel: coalTonnes * reference.coalRateINR, electricity: kWh * reference.gridRateINR, process: hotspots.process * reference.processRateINR, waste: wasteTonnes * reference.wasteRateINR },
    wasteTonnes,
...
export const interventions: Intervention[] = [
  { id: 'waste-heat', name: 'Waste heat recovery', category: 'Energy efficiency', description: 'Recover process heat and displace purchased thermal energy with a heat-recovery system.', sectors: ['Steel', 'Cement', 'Chemicals'], source: 'fuel', reductionRate: .18, costSavingRate: .18, capex: 85000000, annualOpex: 2500000, duration: '6–9 months', complexity: 'Medium', compatibleWith: ['solar', 'motor-efficiency', 'waste-recovery', 'clinker', 'material-substitution'], addressableRef: 0, steps: ['Complete a thermal energy audit', 'Size the recovery system', 'Install heat exchangers and metering', 'Commission and monitor displaced fuel'] },
  { id: 'solar', name: 'On-site solar generation', category: 'Renewable energy', description: 'Replace a portion of grid electricity with on-site solar generation.', sectors: sectors, source: 'electricity', reductionRate: .22, costSavingRate: .20, capex: 120000000, annualOpex: 1800000, duration: '4–6 months', complexity: 'Medium', compatibleWith: ['waste-heat', 'boiler', 'waste-recovery', 'clinker', 'material-substitution'], addressableRef: 0, steps: ['Assess usable rooftop and land', 'Validate grid interconnection', 'Install and commission the array', 'Meter self-consumed generation'] },
  { id: 'motor-efficiency', name: 'High-efficiency drives', category: 'Energy efficiency', description: 'Upgrade motors and variable-speed drives to reduce electricity demand.', sectors: sectors, source: 'electricity', reductionRate: .12, costSavingRate: .12, capex: 28000000, annualOpex: 400000, duration: '2–4 months', complexity: 'Low', compatibleWith: ['waste-heat', 'boiler', 'waste-recovery', 'clinker', 'material-substitution'], addressableRef: 0, steps: ['Log operating load profiles', 'Select high-efficiency motors', 'Upgrade drives during shutdown', 'Validate energy savings'] },
  { id: 'boiler', name: 'Boiler optimisation', category: 'Operational improvement', description: 'Improve combustion settings and maintenance practices with existing equipment.', sectors: ['Steel', 'Chemicals', 'Textiles'], source: 'fuel', reductionRate: .06, costSavingRate: .06, capex: 0, annualOpex: 600000, duration: '2–4 weeks', complexity: 'Low', compatibleWith: ['solar', 'motor-efficiency', 'waste-recovery', 'material-substitution'], addressableRef: 0, steps: ['Measure excess oxygen', 'Tune air-to-fuel ratio', 'Repair insulation and steam leaks', 'Monitor fuel per unit of output'] },
  { id: 'clinker', name: 'Clinker substitution', category: 'Process innovation', description: 'Replace a share of clinker with suitable supplementary cementitious materials.', sectors: ['Cement'], source: 'process', reductionRate: .16, costSavingRate: .14, capex: 55000000, annualOpex: 1500000, duration: '6–12 months', complexity: 'High', compatibleWith: ['waste-heat', 'solar', 'motor-efficiency', 'waste-recovery'], addressableRef: 0, steps: ['Qualify substitute materials', 'Validate product performance', 'Adapt blending and handling', 'Document the revised clinker ratio'] },
  { id: 'waste-recovery', name: 'Waste & water recovery', category: 'Circularity', description: 'Recover usable materials and improve effluent treatment to reduce waste-related emissions.', sectors: sectors, source: 'waste', reductionRate: .30, costSavingRate: .24, capex: 32000000, annualOpex: 950000, duration: '4–8 months', complexity: 'Medium', compatibleWith: ['waste-heat', 'solar', 'motor-efficiency', 'boiler', 'clinker', 'material-substitution'], addressableRef: 0, steps: ['Characterise waste streams', 'Assess recovery opportunities', 'Install segregation and treatment', 'Track recovery and disposal volumes'] },
  { id: 'material-substitution', name: 'Recycled feedstock substitution', category: 'Circularity', description: 'Replace a share of virgin feedstock with recovered or secondary material from a declared stream.', sectors: sectors, source: 'process', reductionRate: .11, costSavingRate: .09, capex: 24000000, annualOpex: 700000, duration: '3\u20136 months', complexity: 'Medium', compatibleWith: ['waste-heat', 'solar', 'motor-efficiency', 'boiler', 'waste-recovery'], requiresMaterial: true, addressableRef: 0, steps: ['Map incoming material streams and recovered fractions', 'Qualify a secondary source against product specification', 'Trial a substitution rate on one line', 'Track substituted tonnage and avoided virgin purchase'] },
];
const median = (xs: number[]) => { const v = [...xs].sort((a, b) => a - b), m = Math.floor(v.length / 2); return v.length % 2 ? v[m] : (v[m - 1] + v[m]) / 2; };
...
interventions.forEach(i => {
  const addressable = factories.filter(f => f.baseline !== null && i.sectors.includes(f.sector)).map(f => f.hotspots[i.source] * i.reductionRate);
