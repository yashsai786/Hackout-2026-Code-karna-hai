...
function loadStored(): { factories?: Factory[]; ledger?: LedgerEntry[]; intake?: IntakeRecord[]; inbox?: InboxItem[] } | null {
  try {
...
export const SessionProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [factories, setFactories] = useState<Factory[]>(() => stored?.factories ?? structuredClone(fixtureFactories));
  ...
  useEffect(() => {
    try { localStorage.setItem(STORE_KEY, JSON.stringify({ factories, ledger, intake, inbox })); }
    catch { /* quota or private mode: the app still works, it just will not survive a reload */ }
  ...
  const value: State = { factories, ledger, intake, inbox, settings,
    ...
    addFactory: (name, city, state, sector) => {
      ...
      setFactories(fs => [...fs, { id, name: name.trim(), city: city.trim(), state: state.trim(), sector,
        ...
        costs: { fuel: 0, electricity: 0, process: 0, waste: 0 }, hotspots: { fuel: 0, electricity: 0, process: 0, waste: 0 }, history: [], materials: [],
