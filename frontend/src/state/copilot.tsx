...
type CopilotState = {
  turns: CopilotTurn[]; running: boolean; streaming: string; toolNow: string;
  mode: CopilotMode; pending: PendingAction | null; error: string; announcement: string;
  send: (input: string) => void; stop: () => void; clear: () => void;
  confirmPending: () => void; declinePending: () => void;
};
...
export const CopilotProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => { sessionRef.current = { factories, ledger, intake }; }, [factories, ledger, intake]);
  ...
  const mode: CopilotMode = !connected || !defaultModel ? 'disconnected' : toolSupport === 'no' ? 'narrated' : 'agent';

  // Warm the catalogue when the panel opens, not only when the Settings dialog does.
  useEffect(() => { if (copilotOpen && connected) ensureModels(); }, [copilotOpen, connected, ensureModels]);
  ...
  const drive = useCallback(async (wire: ChatMessage[]) => {
    if (runningRef.current) return;
    runningRef.current = true;
    setRunning(true); setError(''); setStreaming(''); setToolNow('');
    ...
    const chat = (messages: ChatMessage[], tools: ToolDef[] | undefined, signal?: AbortSignal) => {
      setStreaming('');
      return runChat({ messages, tools, signal, onEvent: e => { if (e.type === 'text') setStreaming(prev => prev + e.delta); } });
    };

    try {
      const outcome = await runAgent(wire, {
        chat, ctx: buildCtx(), signal: controller.signal, toolsEnabled: mode === 'agent',
        onToolStart: run => {
          setToolNow(spoken(run.name));
          const now = Date.now();
          if (now - lastSpoken > 500) { lastSpoken = now; setAnnouncement(`Running ${spoken(run.name)}.`); }
        },
        onToolEnd: run => { runs.push(run); setToolNow(''); },
      });
      wireRef.current = outcome.messages;
      const figures = runs.filter(r => r.ok).length;
      setTurns(t => [...t, { id: uid(), role: 'assistant', text: outcome.text, toolRuns: runs, stopped: outcome.stopped, narrated: mode === 'narrated' }]);
      setAnnouncement(outcome.stopped ? 'Generation stopped. Partial answer kept.' : `Answer complete. ${figures} tool ${figures === 1 ? 'result' : 'results'} cited.`);
    } catch (e) {
      ...
      setRunning(false); setStreaming(''); setToolNow('');
  ...
  const send = useCallback((input: string) => {
    ...
    const system: ChatMessage = { role: 'system', content: buildSystemPrompt({ ...sessionRef.current, toolsEnabled: mode === 'agent' }) };
  ...
  const clear = useCallback(() => {
    ...
    wireRef.current = []; pendingRef.current.clear();
    setTurns([]); setPending(null); setError(''); setStreaming('');
  ...
  const confirmPending = useCallback(() => {
    ...
    setPending(null);
    const wire: ChatMessage[] = [...wireRef.current, { role: 'user', content: `Confirmed. Execute record_estimate with confirmation_token "${entry.token}".` }];
    wireRef.current = wire;
    void drive(wire);
  }, [pending, drive]);
  ...
  const declinePending = useCallback(() => {
    ...
    setPending(null);
    wireRef.current = [...wireRef.current, { role: 'user', content: 'Declined. Do not record that estimate.' }];
    setTurns(t => [...t, { id: uid(), role: 'assistant', text: 'Understood — nothing was recorded.', toolRuns: [] }]);
    setAnnouncement('Proposal declined. Nothing was recorded.');
  }, [pending]);

  const value: CopilotState = { turns, running, streaming, toolNow, mode, pending, error, announcement, send, stop, clear, confirmPending, declinePending };
  return <Ctx.Provider value={value}>{children}</Ctx.Provider>;
};

export const useCopilot = () => { const c = useContext(Ctx); if (!c) throw new Error('CopilotProvider missing'); return c; };
